...
class AcquirerCredentials(BaseModel):
    """
    How we interact with the acquirer
    """ # noqa: E501
    type: StrictStr = Field(description="Type of credentials")
    key1: Annotated[str, Field(min_length=1, strict=True)] = Field(description="A key that can be used as a token")
    key2: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Can be used alongside key1 as credentials")
    __properties: ClassVar[List[str]] = ["type", "key1", "key2"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TOKEN', 'PROFILE_ID', 'WEBHOOK', 'SECRET', 'CLIENT_CREDENTIALS']):
            raise ValueError("must be one of enum values ('TOKEN', 'PROFILE_ID', 'WEBHOOK', 'SECRET', 'CLIENT_CREDENTIALS')")
        return value
