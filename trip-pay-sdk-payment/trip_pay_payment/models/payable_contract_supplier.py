...
class PayableContractSupplier(BaseModel):
    """
    Details of the booking entries that went in for one particular supplier directly.
    """ # noqa: E501
    identifier: Optional[UUID] = Field(default=None, description="Unique identifier used to track the contract. Will auto-generate on if doesn't exist.")
    supplier_identifier: UUID = Field(description="Supplier identifier ", alias="supplierIdentifier")
    supplier_identifier_type: StrictStr = Field(description="Indicates whether the supplierIdentifier can be found internally or is mapped via an external identifier", alias="supplierIdentifierType")
    contract_item_list: Annotated[List[PayableContractSupplierItem], Field(min_length=1, max_length=2147483647)] = Field(alias="contractItemList")
    ...
    external_transaction_identifier: Optional[StrictStr] = Field(default=None, description="External transaction identifier populated when agent responsible for acquiring", alias="externalTransactionIdentifier")
    ...
    __properties: ClassVar[List[str]] = ["identifier", "supplierIdentifier", "supplierIdentifierType", "contractItemList", "externalSupplierBookingCode", "externalTransactionIdentifier", "metadata"]

    @field_validator('supplier_identifier_type')
    def supplier_identifier_type_validate_enum(cls, value):
        """Validates the enum"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in contract_item_list (list)
        _items = []
        if self.contract_item_list:
            for _item_contract_item_list in self.contract_item_list:
                if _item_contract_item_list:
                    _items.append(_item_contract_item_list.to_dict())
            _dict['contractItemList'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "supplierIdentifierType": obj.get("supplierIdentifierType"),
            "contractItemList": [PayableContractSupplierItem.from_dict(_item) for _item in obj["contractItemList"]] if obj.get("contractItemList") is not None else None,
