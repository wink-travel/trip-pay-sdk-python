...
class BeneficiaryCharge(BaseModel):
    """
    A variable charge can be a fixed amount or a percentage.
    """ # noqa: E501
    type: StrictStr = Field(description="The type of charge")
    percent: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="A percentage of the total stay amount for an early checkin or late checkout")
    __properties: ClassVar[List[str]] = ["type", "percent"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PERCENTAGE', 'REMAINDER']):
            raise ValueError("must be one of enum values ('PERCENTAGE', 'REMAINDER')")
        return value
