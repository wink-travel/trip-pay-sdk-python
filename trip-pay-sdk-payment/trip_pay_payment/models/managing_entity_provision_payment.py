...
class ManagingEntityProvisionPayment(BaseModel):
    """
    ManagingEntityProvisionPayment
    """ # noqa: E501
    subscription_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Stripe subscription ID", alias="subscriptionId")
    invoice_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Latest Stripe invoice ID", alias="invoiceId")
    customer_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Stripe customer ID", alias="customerId")
    customer_email: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Stripe customer ID", alias="customerEmail")
    customer_phone: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Stripe customer ID", alias="customerPhone")
    price_lookup_key: StrictStr = Field(description="Stripe price lookup key", alias="priceLookupKey")
    status: StrictStr = Field(description="Stripe subscription status")
    invoice_status: StrictStr = Field(description="Stripe latest invoice status", alias="invoiceStatus")
    created_at: datetime = Field(description="Time subscription was created", alias="createdAt")
    updated_at: datetime = Field(description="Time subscription was last updated", alias="updatedAt")
    canceled_at: datetime = Field(description="Time subscription was canceled", alias="canceledAt")
    trial_expires: datetime = Field(description="Time subscription was last updated", alias="trialExpires")
    metadata: Optional[Dict[str, StrictStr]] = Field(default=None, description="Optional information we receive from Stripe that we want to save.")
    active: Optional[StrictBool] = None
    __properties: ClassVar[List[str]] = ["subscriptionId", "invoiceId", "customerId", "customerEmail", "customerPhone", "priceLookupKey", "status", "invoiceStatus", "createdAt", "updatedAt", "canceledAt", "trialExpires", "metadata", "active"]

    @field_validator('price_lookup_key')
    def price_lookup_key_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['CREATOR_FREE', 'CREATOR_MONTHLY', 'CREATOR_YEARLY', 'STARTER_FREE', 'STARTER_MONTHLY', 'STARTER_YEARLY', 'GROWTH_FREE', 'GROWTH_MONTHLY', 'GROWTH_YEARLY', 'ELITE_FREE', 'ELITE_MONTHLY', 'ELITE_YEARLY', 'ENTERPRISE']):
            raise ValueError("must be one of enum values ('CREATOR_FREE', 'CREATOR_MONTHLY', 'CREATOR_YEARLY', 'STARTER_FREE', 'STARTER_MONTHLY', 'STARTER_YEARLY', 'GROWTH_FREE', 'GROWTH_MONTHLY', 'GROWTH_YEARLY', 'ELITE_FREE', 'ELITE_MONTHLY', 'ELITE_YEARLY', 'ENTERPRISE')")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACTIVE', 'CANCELED', 'INCOMPLETE', 'INCOMPLETE_EXPIRED', 'PAST_DUE', 'PAUSED', 'TRIALING', 'UNPAID']):
            raise ValueError("must be one of enum values ('ACTIVE', 'CANCELED', 'INCOMPLETE', 'INCOMPLETE_EXPIRED', 'PAST_DUE', 'PAUSED', 'TRIALING', 'UNPAID')")
        return value

    @field_validator('invoice_status')
    def invoice_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DRAFT', 'OPEN', 'PAID', 'UNCOLLECTIBLE', 'VOID']):
            raise ValueError("must be one of enum values ('DRAFT', 'OPEN', 'PAID', 'UNCOLLECTIBLE', 'VOID')")
        return value
