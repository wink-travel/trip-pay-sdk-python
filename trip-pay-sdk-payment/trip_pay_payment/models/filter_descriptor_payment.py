...
class FilterDescriptorPayment(BaseModel):
    """
    FilterDescriptorPayment
    """ # noqa: E501
    var_field: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Field name to filter on", alias="field")
    operator: StrictStr = Field(description="Filter operator to use on field")
    value: Optional[Any]
    ignore_case: Optional[StrictBool] = Field(default=None, description="Make filter comparison case insensitive. Default: Case sensitive ", alias="ignoreCase")
    __properties: ClassVar[List[str]] = ["field", "operator", "value", "ignoreCase"]

    @field_validator('operator')
    def operator_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['neq', 'eq', 'contains', 'doesnotcontain', 'isnull', 'isnotnull', 'isempty', 'isnotempty', 'startswith', 'endswith', 'gte', 'lte', 'gt', 'lt', 'in', 'nin']):
            raise ValueError("must be one of enum values ('neq', 'eq', 'contains', 'doesnotcontain', 'isnull', 'isnotnull', 'isempty', 'isnotempty', 'startswith', 'endswith', 'gte', 'lte', 'gt', 'lt', 'in', 'nin')")
        return value
