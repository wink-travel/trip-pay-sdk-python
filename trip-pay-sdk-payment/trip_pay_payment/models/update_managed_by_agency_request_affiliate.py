...
class UpdateManagedByAgencyRequestAffiliate(BaseModel):
    """
    UpdateManagedByAgencyRequestAffiliate
    """ # noqa: E501
    managing_entity_identifier: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Existing user account managingEntityIdentifier to make manager", alias="managingEntityIdentifier")
    commission_in_percent: Union[StrictFloat, StrictInt] = Field(description="Agency commission", alias="commissionInPercent")
    rules: Optional[ManagedByEntityRulesAffiliate] = Field(default=None, description="Optional rules for expiration date etc when agency is no longer managing this entity.")
    __properties: ClassVar[List[str]] = ["managingEntityIdentifier", "commissionInPercent", "rules"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "rules": ManagedByEntityRulesAffiliate.from_dict(obj["rules"]) if obj.get("rules") is not None else None
        })
        return _obj
