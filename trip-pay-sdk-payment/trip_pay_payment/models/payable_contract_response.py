...
class PayableContractResponse(BaseModel):
    """
    Details of the booking entries that went in for one particular supplier directly.
    """ # noqa: E501
    id: StrictStr = Field(description="TripPay supplierContractIdentifier for payment widget")
    trace_id: Optional[StrictStr] = Field(default=None, description="Groups all booking contracts submitted at the same time", alias="traceId")
    supplier_contract_identifier: UUID = Field(description="Unique supplierContractIdentifier used to track the contract. Create a UUID for this purpose.", alias="supplierContractIdentifier")
    supplier_identifier: UUID = Field(description="Supplier supplierContractIdentifier", alias="supplierIdentifier")
    supplier_name: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Supplier name", alias="supplierName")
    external_supplier_identifier: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Track supplier with its external supplier supplierContractIdentifier", alias="externalSupplierIdentifier")
    ...
    external_transaction_identifier: Optional[StrictStr] = Field(default=None, description="External transaction supplierContractIdentifier populated when agent responsible for acquiring", alias="externalTransactionIdentifier")
    total_price: CustomMonetaryAmount = Field(alias="totalPrice")
    total_display_price: CustomMonetaryAmount = Field(alias="totalDisplayPrice")
    total_supplier_price: CustomMonetaryAmount = Field(alias="totalSupplierPrice")
    total_internal_price: CustomMonetaryAmount = Field(alias="totalInternalPrice")
    total_capture_price: CustomMonetaryAmount = Field(alias="totalCapturePrice")
    __properties: ClassVar[List[str]] = ["id", "traceId", "supplierContractIdentifier", "supplierIdentifier", "supplierName", "externalSupplierIdentifier", "externalSupplierBookingCode", "externalTransactionIdentifier", "totalPrice", "totalDisplayPrice", "totalSupplierPrice", "totalInternalPrice", "totalCapturePrice"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "supplierContractIdentifier": obj.get("supplierContractIdentifier"),
