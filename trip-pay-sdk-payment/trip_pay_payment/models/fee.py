...
class Fee(BaseModel):
    ...
    Fee
    """ # noqa: E501
    identifier: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Unique system ID.")
    ...
    __properties: ClassVar[List[str]] = ["identifier", "fee", "type", "description"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACQUIRING', 'FX', 'TAX']):
            raise ValueError("must be one of enum values ('ACQUIRING', 'FX', 'TAX')")
        return value
