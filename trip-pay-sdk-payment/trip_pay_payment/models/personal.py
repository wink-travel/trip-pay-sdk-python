...
class Personal(BaseModel):
    ...
    Personal
    """ # noqa: E501
    gender: Optional[StrictStr] = Field(default=None, description="Identifier the gender of the customer.")
    birth_date: Optional[date] = Field(default=None, description="Indicates the date of birth as indicated in the document, in ISO 8601 prescribed format.", alias="birthDate")
    marital_status: Optional[StrictStr] = Field(default=None, description="Marital status of the traveler.", alias="maritalStatus")
    child_quantity: Optional[StrictInt] = Field(default=None, description="The number of children of the customer.", alias="childQuantity")
    citizenship: Optional[StrictStr] = Field(default=None, description="Name of the (self-professed) country that is clamided for citizenship.")
    ...
    preferred_currency: Optional[StrictStr] = Field(default=None, description="Type of funds preferred for reviewing monetary values, in ISO 4217 codes", alias="preferredCurrency")
    language: Optional[StrictStr] = Field(default=None, description="The primary language of the customer")
    contact_person: Optional[List[Contact]] = Field(default=None, alias="contactPerson")
    phys_chall_name: Optional[List[StrictStr]] = Field(default=None, alias="physChallName")
    pet_info: Optional[List[PetInfoLightweight]] = Field(default=None, alias="petInfo")
    __properties: ClassVar[List[str]] = ["gender", "birthDate", "maritalStatus", "childQuantity", "citizenship", "address1", "address2", "city", "state", "postalCode", "country", "preferredCurrency", "language", "contactPerson", "physChallName", "petInfo"]
    ...
    def marital_status_validate_enum(cls, value):
        ...
        if value not in set(['ANNULLED', 'COHABITATING', 'DIVORCED', 'ENGAGED', 'MARRIED', 'SEPARATED', 'SINGLE', 'WIDOWED', 'UNKNOWN']):
            raise ValueError("must be one of enum values ('ANNULLED', 'COHABITATING', 'DIVORCED', 'ENGAGED', 'MARRIED', 'SEPARATED', 'SINGLE', 'WIDOWED', 'UNKNOWN')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in contact_person (list)
        _items = []
        if self.contact_person:
            for _item_contact_person in self.contact_person:
                if _item_contact_person:
                    _items.append(_item_contact_person.to_dict())
            _dict['contactPerson'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in pet_info (list)
        _items = []
        if self.pet_info:
            for _item_pet_info in self.pet_info:
                if _item_pet_info:
                    _items.append(_item_pet_info.to_dict())
            _dict['petInfo'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "childQuantity": obj.get("childQuantity"),
            ...
            "contactPerson": [Contact.from_dict(_item) for _item in obj["contactPerson"]] if obj.get("contactPerson") is not None else None,
            "physChallName": obj.get("physChallName"),
            "petInfo": [PetInfoLightweight.from_dict(_item) for _item in obj["petInfo"]] if obj.get("petInfo") is not None else None
        })
        return _obj
