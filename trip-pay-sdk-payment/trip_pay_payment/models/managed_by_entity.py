...
class ManagedByEntity(BaseModel):
    """
    ManagedByEntity
    """ # noqa: E501
    identifier: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Entity identifier")
    name: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Name of entity")
    commission_in_percent: Union[StrictFloat, StrictInt] = Field(description="Agency commission", alias="commissionInPercent")
    rules: Optional[ManagedByEntityRules] = Field(default=None, description="Optional rules for expiration date etc when agency is no longer managing this entity.")
    valid: Optional[StrictBool] = Field(default=None, description="Whether managing entity is eligible a commission.")
    __properties: ClassVar[List[str]] = ["identifier", "name", "commissionInPercent", "rules", "valid"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "name": obj.get("name"),
            "commissionInPercent": obj.get("commissionInPercent"),
            "rules": ManagedByEntityRules.from_dict(obj["rules"]) if obj.get("rules") is not None else None,
            "valid": obj.get("valid")
        })
        return _obj
