...
class AccountStatusEntry(BaseModel):
    ...
    """ # noqa: E501
    task_id: UUID = Field(description="Unique task identifier.", alias="taskId")
    type: StrictStr = Field(description="Type of textual entry")
    text: UUID = Field(description="Textual description of .")
    __properties: ClassVar[List[str]] = ["taskId", "type", "text"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['MISSING_NATIONAL_IDENTIFIER']):
            raise ValueError("must be one of enum values ('MISSING_NATIONAL_IDENTIFIER')")
        return value
