...
class AggregateDescriptorPayment(BaseModel):
    """
    AggregateDescriptorPayment
    """ # noqa: E501
    var_field: Optional[StrictStr] = Field(default=None, description="Field to run aggregate function on", alias="field")
    aggregate: Optional[StrictStr] = Field(default=None, description="Aggregate function")
    __properties: ClassVar[List[str]] = ["field", "aggregate"]
    ...
    def aggregate_validate_enum(cls, value):
        ...
        if value not in set(['count', 'sum', 'average', 'min', 'max']):
            raise ValueError("must be one of enum values ('count', 'sum', 'average', 'min', 'max')")
        return value
